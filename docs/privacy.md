...
| Status | Optional: one of five words chosen from a list ("on a road trip", "heading to charge"…), or up to 24 characters you write yourself. A written one is shown to other drivers exactly as you wrote it, with the same cap and the same absence of moderation as a nickname (ADR-0040, amended). |
| A report, and a vote on one | Only if you tap Report, or answer a pin: the kind (police or accident) and where your car was at that moment. Shared with drivers in the same map cell as a pin that carries no id, no name and nothing about the car behind it. The hub keeps the ids of the drivers who say a pin is there and those who say it is gone in memory only, so each counts once and can change their mind, and they go with the pin (ADR-0040). |
...
| Reports | Durable Object memory only | The earlier of 60 to 90 minutes from the last driver who confirmed it and 4 hours from when it was placed, then dropped; gone on hibernation. Never written to storage, like positions |
